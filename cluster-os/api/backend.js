...
// ── In-memory cluster state (shared within a warm lambda instance) ────────────
const workers = new Map();      // id → WorkerRecord
const jobs    = new Map();      // id → JobRecord
const jobQueue = [];            // pending job ids
let   lbRunning = true;         // simulated LB toggle
let   requestCount = 0;

// Seed a couple of demo workers so the dashboard is non-empty on cold start
function seedWorkers() {
  if (workers.size === 0) {
    const w1 = { id: randomUUID(), name: 'worker-0', status: 'healthy', activeJobs: 0, lastHeartbeat: Date.now(), createdAt: Date.now() };
    const w2 = { id: randomUUID(), name: 'worker-1', status: 'healthy', activeJobs: 0, lastHeartbeat: Date.now(), createdAt: Date.now() };
...
function assignQueuedJobs() {
  while (jobQueue.length > 0) {
    ...
    // Simulate processing: complete the job after 3 s
    setTimeout(() => {
...
// Metrics (used by dashboard polling)
app.get('/api/metrics', (_req, res) => {
  const running = Array.from(jobs.values()).filter(j => j.status === 'running').length;
  const circuitBreakerStates = {};
  for (const [id, w] of workers) {
    circuitBreakerStates[id.slice(0, 8)] = w.status === 'healthy' ? 'CLOSED' : 'OPEN';
  }
  sendJson(res, 200, {
    healthyWorkers: getHealthyWorkers().length,
...
app.post('/api/start-lb', (_req, res) => {
  lbRunning = true;
  sendJson(res, 200, { status: 'Load Balancer started (cloud demo)' });
});

app.post('/api/kill-lb', (_req, res) => {
  lbRunning = false;
  sendJson(res, 200, { status: 'Load Balancer stopped (cloud demo)' });
});
...
app.post('/api/start-worker', (_req, res) => {
  ...
  const worker = { id, name: `worker-${idx}`, status: 'healthy', activeJobs: 0, lastHeartbeat: Date.now(), createdAt: Date.now() };
  workers.set(id, worker);
  assignQueuedJobs();
  sendJson(res, 200, { status: `Worker ${worker.name} started (PID: demo)`, worker });
});
...
app.post('/api/kill-worker', (_req, res) => {
  ...
  const removed = workers.get(lastId);
  workers.delete(lastId);
  sendJson(res, 200, { status: `Worker ${removed?.name || lastId} stopped` });
});
...
app.post('/api/submit-job', (req, res) => {
  ...
  if (worker) {
    worker.activeJobs++;
    // Simulate processing: complete after 3 s
    setTimeout(() => {
      if (job.status === 'running') {
        job.status      = 'completed';
...
// ── REST worker/job endpoints (for Render/direct API use) ─────────────────────
...
// Local dev: start server if run directly (not imported by Vercel)
